...
public abstract class PDFExport {
	...
	public static void exportPDF(Horse horse, int depth) {
		try {
			...
			PdfWriter.getInstance(document, new FileOutputStream(horse.getName()
					+ "_ancestors.pdf"));
			document.open();
			document.addTitle("Genealogy Tree");
			Anchor anchor = new Anchor("The Genealogy of the horse "
					+ horse.getName(), catFont);
			Chapter catPart = new Chapter(new Paragraph(anchor), 1);
			createTable(catPart, horse, depth);
			document.add(catPart);
			document.close();
			System.out.println("Exported to file: " + horse.getName()
					+ "_ancestors.pdf");
		} catch (Exception e) {
			e.printStackTrace();
		}
